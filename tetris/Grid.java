...
public class Grid {
	...
	public void checkRows() {
		//for each row, starting from the top (starting with less-full rows means fewer total squares to shift).
		for (int row = 0; row < Grid.HEIGHT; row++){ 
			...
			for (int col = 0; col < Grid.WIDTH; col++){ 
				...
				//skip checking subsequent squares if we know the row isn't full.
				if (!isFull) break; 
			}
			if (isFull) removeRow(row);
		}
	}

	/**
	 * Removes a row by shifting all of the squares above it down by one.
	 *  @param row - row to remove.
	 */
	private void removeRow(int row){
		//move from current to top row.
		while (row >0){
			//change the color of each square to that of the square above it.
			for (int col = 0; col < WIDTH; col++){
				Color newColor = board[row-1][col].getColor();
